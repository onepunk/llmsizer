...
export default function App() {
  ...
  const { results, allFits, loading, error, totalModels } = useModels(hw.system, filters)
  ...
  const selectedFit = selectedModelName
    ? results.find((fit) => fit.model.name === selectedModelName) ?? null
    : null

  // Filtering may remove the open model. Clear the selection instead of
  // allowing it to reappear unexpectedly when the filter is later removed.
  useEffect(() => {
    if (selectedModelName && !loading && !selectedFit) setSelectedModelName(null)
  }, [selectedModelName, selectedFit, loading])

  // Resolve comparisons against the unfiltered analysis. Search and fit
  // filters should narrow the table without making selected comparisons vanish.
  const compareFits = useMemo(() => {
    if (compare.length === 0) return []
    const byName = new Map(allFits.map((r) => [r.model.name, r]))
    return compare
  ...
  // The weekly model database can retire repositories. Drop stale comparison
  // names after loading so an old shared URL cannot consume all compare slots
  // with models that no longer exist.
  useEffect(() => {
    if (loading) return
    const available = new Set(allFits.map((fit) => fit.model.name))
    setCompare((prev) => {
  ...
  const closeDetail = useCallback(() => setSelectedModelName(null), [])
      ...
      <header className="app-header">
        ...
        <div className="header-actions">
          <a
            ...
            aria-label="View llmsizer on GitHub"
          >
