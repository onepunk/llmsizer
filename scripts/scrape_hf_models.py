...
# ---------------------------------------------------------------------------
# Pre-quantized weight sizing (llmsizer fork)
# ---------------------------------------------------------------------------
#
# AWQ/GPTQ/MLX/compressed-tensors repos store packed weights: safetensors
# .total reports the packed int32 element count, not real parameter count.
# A 80B-param AWQ-4bit model reports ~14.4B via the API. To size memory
# correctly we sum the actual weight-file sizes from the HF tree API.

# Native quant → bytes-per-param (includes scales/zeros overhead).
# Tuned so weight_gb * 1e9 / BYTES_PER_PARAM matches published param counts
# (e.g. Qwen3-Next-80B AWQ-4bit: 48.3 GB / 0.60 ≈ 80B).
PREQUANT_BYTES_PER_PARAM = {
    "AWQ-4bit": 0.60,
    "AWQ-8bit": 1.10,
    "GPTQ-Int4": 0.60,
    "GPTQ-Int8": 1.10,
...
def fetch_weight_size_gb(repo_id: str) -> float | None:
    """Sum .safetensors / .gguf / .bin file sizes in the repo. Returns None
    if the API fails or no weight files are present."""
    ...
    try:
        ...
        print(f"  ⚠ Tree fetch failed for {repo_id}: {e}", file=sys.stderr)
        return None
    ...
    for f in tree:
        ...
        if path.endswith(".safetensors.index.json"):
            continue
        if not path.endswith((".safetensors", ".gguf", ".bin", ".pt", ".pth")):
            continue
...
def is_prequantized_format(model_format: str, quant_label: str) -> bool:
    fmt = (model_format or "").lower()
    if fmt in ("awq", "gptq", "mlx", "bnb", "exl2"):
        return True
    if quant_label.startswith(("AWQ", "GPTQ", "MLX", "BNB", "EXL2")):
        return True
    return False


def apply_prequant_sizing(
    repo_id: str,
    model_format: str,
    quant_label: str,
    total_params: int,
) -> tuple[int, float | None, float | None, float | None, float | None]:
    """For pre-quantized repos, override total_params and the RAM/VRAM
    estimates using the real file sizes. Returns (total_params,
    weight_gb, min_ram, rec_ram, min_vram); any of the last four may be
    None if the tree fetch fails (caller should fall through to the
    safetensors-based estimate)."""
    if not is_prequantized_format(model_format, quant_label):
        return total_params, None, None, None, None
    weight_gb = fetch_weight_size_gb(repo_id)
    if weight_gb is None:
        return total_params, None, None, None, None
    bpp = PREQUANT_BYTES_PER_PARAM.get(quant_label, 0.60)
    real_params = int(weight_gb * 1e9 / bpp)
    # Memory estimates: weight size + KV/activation/OS overhead.
    min_vram = round(weight_gb * 1.10 + 0.5, 1)
    min_ram = round(max(weight_gb * 1.20 + 0.5, 1.0), 1)
    rec_ram = round(max(weight_gb * 2.00 + 0.5, 2.0), 1)
    return real_params, weight_gb, min_ram, rec_ram, min_vram
...
def scrape_model(repo_id: str) -> dict | None:
    ...
    # weight-file sizes from the HF tree API and derive memory from that.
    weight_gb: float | None = None
    real_params, wgb, min_ram_pq, rec_ram_pq, min_vram_pq = apply_prequant_sizing(
    ...
    # Ground-truth weight size for pre-quantized repos (used by the UI
    # engine so it doesn't re-apply a Q4_K_M formula on packed weights).
    if weight_gb is not None:
        result["weight_gb"] = weight_gb
